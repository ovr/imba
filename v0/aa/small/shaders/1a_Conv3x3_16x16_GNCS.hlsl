// Fused Conv3x3 + GN partial accumulation: IN_CH=16, OUT_CH=16, stride=1
...
void main(uint3 dtid : SV_DispatchThreadID, uint3 gid : SV_GroupID, uint gi : SV_GroupIndex)
{
    ...
    if (valid)
    {
        ...
        // Write conv output (no activation — GNApply handles that)
    ...
    // Accumulate GN partial statistics across the 8x8 tile
